...
class Meteor:
    def __init__(self):
        ...
        try:
            ...
            if not os.path.exists(jar_path):
                raise FileNotFoundError(f"meteor-1.5.jar not found at {jar_path}")

            meteor_cmd = ['java', '-jar', f'-Xmx{mem}', jar_path,
                         '-', '-', '-stdio', '-l', 'en', '-norm']
            ...
            self.meteor_p = subprocess.Popen(
                ...
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                universal_newlines=True  # Use text mode for better performance
            )
            ...
            if hasattr(self, 'meteor_p') and self.meteor_p:
                self.close()
            raise RuntimeError(f"Failed to initialize Meteor: {str(e)}")
    ...
    def close(self):
        ...
        if atexit and hasattr(atexit, 'unregister'):
            atexit.unregister(self.close)
